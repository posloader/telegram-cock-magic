...
async def process_love_words(event: NewMessage.Event):
    await client.edit_message(event.peer_id.user_id, event.message.id, 'люблю')
    await asyncio.sleep(1)
    await client.edit_message(event.peer_id.user_id, event.message.id, 'люблю сосать')
    await asyncio.sleep(1)
    await client.edit_message(event.peer_id.user_id, event.message.id, 'люблю сосать жирные')
    await asyncio.sleep(1)
    await client.edit_message(event.peer_id.user_id, event.message.id, 'люблю сосать жирные члены')
    await asyncio.sleep(1)
    await client.edit_message(event.peer_id.user_id, event.message.id, 'люблю сосать жирные члены😋')
